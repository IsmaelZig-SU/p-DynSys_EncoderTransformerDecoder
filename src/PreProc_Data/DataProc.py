...
class StackedSequenceDataset(Dataset):
    def __init__(self, statedata, args_dict):
        ...
        self.load_epoch = args_dict['load_epoch']
    ...
    def filter_tensor(self, tensor, a, b):
        new_samples = []
        old_samples = []

        # Iterate through the subtensors along the first dimension
        for i in range(tensor.shape[0]):
            sub_tensor = tensor[i, ...]
            
            # Check if the last two elements are [a, b]
            if torch.equal(sub_tensor[-1, -2:], torch.tensor([a, b])):
                new_samples.append(sub_tensor)
            else:
                old_samples.append(sub_tensor)

        # Stack the collected new and old samples into tensors
        new_samples = torch.stack(new_samples, dim=0)
        old_samples = torch.stack(old_samples, dim=0)
        print(old_samples.shape, new_samples.shape)

        # Randomly select 1/10 of the old samples
        num_non_matching_samples = old_samples.shape[0] // 10
        random_indices = torch.randperm(old_samples.shape[0])[:num_non_matching_samples]
        selected_old_samples = old_samples[random_indices, :, :]

        # Concatenate new_samples with the selected old_samples
        final_tensor = torch.cat((new_samples, selected_old_samples), dim=0)
    ...
    def stack_data(self):
        ...
        #     Phi_seq = self.filter_tensor(Phi_seq, 120/140, 1.22/1.5)
        #     Phi_nn = self.filter_tensor(Phi_nn, 120/140, 1.22/1.5)
            #print(Phi_nn.shape, Phi_seq.shape)
    ...
    def __getitem__(self, i):
        ...
        '''
        # print(self.stacked_Phi_seq.shape, self.stacked_Phi_nn.shape)
